...
// import logo from "../assets/nicolas-logo.jpeg";
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 shrink-0 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 px-4 py-6 flex flex-col h-full transition-colors duration-300">

      {/* LOGO */}
      <div className="flex items-center justify-center mb-10">
        <h1 className="text-2xl font-extrabold text-blue-600 dark:text-blue-400 tracking-tight">NICOLAS</h1>
      </div>
      ...
      <nav className="space-y-2 flex-1">
        {navItems.map(({ name, path, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-5 h-5 transition-colors ${location.pathname === path
      ...
      <div className="mt-auto pt-6 border-t border-gray-200 dark:border-gray-800 space-y-2">
            ...
            <div className="absolute bottom-full left-0 w-full mb-2 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 py-1 z-10 max-h-48 overflow-y-auto">
              {LANGUAGES.map(lang => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 ${language === lang.code ? 'text-blue-600 font-bold' : 'text-gray-600 dark:text-gray-300'}`}
                >
                  {lang.label}
                </button>
              ))}
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${isLoggedIn
            ? "text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
            : "text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20"
            }`}
        >
          {isLoggedIn ? <LogOut className="w-5 h-5" /> : <LogIn className="w-5 h-5" />}
          <span>{isLoggedIn ? t("logout") : "Login"}</span>
        </button>
